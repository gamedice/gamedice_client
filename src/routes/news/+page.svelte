  ...
  <div class="container_for_news flex justify-center flex-wrap p-5 m-5">
    {#each paginatedItems as item}
      <Card class="one_card m-10 dark:bg-gray-900 dark:border-gray-700">
        ...
        <h5 class="title_card_news mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white whitespace-nowrap text-ellipsis overflow-hidden">
          {item.title}
        </h5>
        <p class="h-40 mb-3 font-normal text-gray-700 dark:text-gray-400 leading-tight overflow-hidden text-justify">
          {item.contain}
        </p>
        <Button color="light" href="/news/{item.id}" class="button_to_news_item">
          Подробнее <svg
            ...
            stroke-width="1.5"
            stroke="currentColor"
            class="w-5 h-5 ml-2"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3"/></svg>
        </Button>
      </Card>
    {/each}
  </div>
