...
<Card size="xl" class="m-auto dark:bg-gray-900 dark:border-gray-700">
  {#if !form?.message_success}
    ...
    <form method="POST" action="?/new_post" enctype="multipart/form-data" use:enhance>
      ...
      <Input rows="1" class="id_user_to_post mb-4" placeholder="IdUser" name="user" />
      <input
        type="file"
  ...
  {:else}
    <h1 class="text-2xl font-bold text-green-600">Успешно, <a href="/blog" class="link_after_add_post">перейти к блогу</a></h1>
  {/if}
</Card>
