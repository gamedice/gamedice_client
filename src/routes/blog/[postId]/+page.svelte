...
<div>
    ...
    <div class="mb-7">
        ...
        <div class="text-sm">{comm.date.split("-")[2].split("T")[0]}.{comm.date.split("-")[1]}.{comm.date.split("-")[0]} {comm.date.split("T")[1].substring(0, 5)}</div>
        <div>{comm.text}</div>
        </Card>
    </div>
    {/each}  
    ...
    <div class="mt-7">
        ...
        <form method="POST" action="?/new_comm" use:enhance>
            {#if !form?.message_success}
                ...
                <Textarea class="mb-4" rows="4" placeholder="Оставить комментарий" name="text"></Textarea>
